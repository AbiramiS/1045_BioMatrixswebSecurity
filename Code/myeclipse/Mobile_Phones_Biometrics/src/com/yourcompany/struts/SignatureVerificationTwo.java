...
public class SignatureVerificationTwo extends org.apache.struts.action.Action 
{
	Connection con,con1;
	Statement stmt;
	ResultSet rs;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception,IOException {
    	...
    	String[] proof112;
    	String image="";
    	String imgcontent="",imgpathnew="",dbaccno="";
    	String sorry="***",user="***";
    	PrintWriter pw=response.getWriter();
    	String newuname="",uploadedFile1="",useridstatus="",response1="",positionvalues="",positionvaluesnew="",newupass="",ugend="",umail="",ucontno="",proof="",proof66="",uuname="",uupass="",passcheck="",passchosen="";
    	...
    	if(set1!=null){
    		...
    		while(itr1.hasNext()){
    			...
    			else if(fieldname.equals("useridstatus")){
    				useridstatus1=(String[])ht1.get(fieldname);
    				for(int i=0;i<useridstatus1.length;i++)
    				{
    					useridstatus=useridstatus1[i].toString();
    				}
    			}
    		}
    		//---------------------------------------
    		...
    		File f=new File("SIGNATURE IMAGES2/"+useridstatus+"encryptedvalues"+".txt");
    		if(!f.exists()){
    		f.createNewFile();
    		...
    		File f1=new File("SIGNATURE IMAGES2/"+useridstatus+"xypositions"+".txt");
    		if(!f1.exists()){
    		f1.createNewFile();
            ...
            File ffnew = new File("SIGNATURE IMAGES2/"+useridstatus+".jpg");
    		...
    		try {
                DataIO.writeData(sigData1, useridstatus, "sig2");
                DataIO.writeData(sigData1Norm, useridstatus, "sig2Norm");
            } catch (IOException e1) {
            }
            try {
                byte[] dig = messageDigestCalculator.computeDigest(useridstatus, "sig2Norm");
                
                System.out.println("DIG is------"+dig);
                messageDigestCalculator.recordDigestToDisk(dig, useridstatus, "sig2NormDigest");
    	...
    	rs=stmt.executeQuery("select * from MPB_CLIENTREGISTER where USERID='"+useridstatus+"'");
		while(rs.next()){
			dbaccno=rs.getString("ACCNO");
		}
            ...
            // Else, Decoding
                ...
                if( decodabet[ theByte & 0x7f ] > WHITE_SPACE_ENC ) {
                    ...
                    if( position >= bufferLength ) { // Enough to output.
                        ...
                        int len = Base64ab.decode4to3( buffer, 0, b4, 0, options );
                        out.write( b4, 0, len );
                        position = 0;
                    }   // end if: enough to output
